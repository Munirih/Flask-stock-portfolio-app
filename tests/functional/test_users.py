...
def test_get_registration_page(test_client):
    ...
    assert b'Flask Stock Portofolio App' in response.data
...
def test_valid_registration(test_client):
    ...
    with mail.record_messages() as outbox:
        ...
        assert response.status_code == 200
        assert b'Thanks for registering, niyammadi@gmail.com! Please check your email address' in response.data
        assert b'Flask Stock Portfolio App' in response.data
        assert len(outbox) == 1
        assert outbox[0].subject == 'Flask Stock Portfolio App - Confirm Your Email Address'
        assert outbox[0].sender == 'flaskcoursetest@gmail.com'
        assert outbox[0].recipients[0] == 'niyammadi@gmail.com'
...
def test_invalid_registration(test_client):
    ...
    """
    response = test_client.post('/users/register',
                                data={'email': 'nganyadem@yahoo.fr',
                                      'password': ''},  
                                follow_redirects=True)
    assert response.status_code == 200
    assert b'Thanks for registering, nganyadem@yahoo.fr!' not in response.data
    assert b'Flask Stock Portfolio App' in response.data
...
def test_duplicate_registration(test_client):
    ...
    test_client.post('/users/register',
                     ...
                     follow_redirects=True)
    response = test_client.post('/users/register',
                                data={'email': 'niyammadi@gmail.com',   # Duplicate email address
                                      'password': 'FlaskIsStillGreat!'},
                                follow_redirects=True)
    assert response.status_code == 200
    assert b'Thanks for registering, niyammadi@gmail.com!' not in response.data
    assert b'Flask Stock Portfolio App' in response.data
    assert b'ERROR! Email (niyammadi@gmail.com) already exists.' in response.data
...
def test_valid_login_and_logout(test_client, register_default_user):
    ...
    response = test_client.post('/users/login',
                                ...
                                follow_redirects=True)
    assert response.status_code == 200
    assert b'Thanks for logging in, niyammadi@gmail.com!' in response.data
    assert b'Flask Stock Portfolio App' in response.data
...
def test_user_profile_logged_in(test_client, log_in_default_user):
    ...
    assert b'Email: niyammadi@gmail.com' in response.data
...
def test_user_profile_not_logged_in(test_client):
    ...
    assert b'Email: niyammadi@gmail.com' not in response.data
    assert b'Please log in to access this page.' in response.data
...
def test_confirm_email_valid(test_client):
    ...
    token = confirm_serializer.dumps('niyammadi@gmail.com', salt='email-confirmation-salt')
...
def test_confirm_email_already_confirmed(test_client):
    """
    GIVEN a Flask application
    WHEN the '/users/confirm/<token>' page is requested (GET) with a valid data
...
def test_post_password_reset_via_email_page_valid(test_client, confirm_email_default_user):
    ...
    with mail.record_messages() as outbox:
        ...
        assert 'flaskcoursetest@gmail.com' in outbox[0].html
...
def test_post_password_reset_via_email_page_invalid(test_client):
    ...
    with mail.record_messages() as outbox:
        response = test_client.post('/users/password_reset_via_email',
                                    data={'email': 'linkmunirih@gmail.com'},
                                    follow_redirects=True)
        assert response.status_code == 200
        assert len(outbox) == 0
...
def test_get_password_reset_valid_token(test_client):
    ...
    token = password_reset_serializer.dumps('niyammadi@gmail.com', salt='password-reset-salt')
...
def test_user_profile_logged_in_email_confirmed(test_client, confirm_email_default_user):
    ...
    assert b'Flask Stock Portfolio App' in response.data
    assert b'User Profile' in response.data
    assert b'Email: linkmunirih@gmail.com' in response.data
    assert b'Account Statistics' in response.data
    assert b'Joined on' in response.data
